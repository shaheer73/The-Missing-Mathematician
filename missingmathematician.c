...
//function used to display the equation on seven segment display
void display(int n1, int n2, int n3, char o1, char o2){
    Initialize7Segment();
    uint8_t operation = 0b0000000;
    
    //display the first number for 2 seconds
    Display7SegmentNumber(n1);
    HAL_Delay(2000);
    //display the first operator for 2 seconds
    if(o1 == '+') {
        operation = 0b0100001;
    } 
    else if(o1 == '-') {
    ...
    Display7Segment(operation);
    HAL_Delay(2000);
    //display the second number for 2 seconds
    Display7SegmentNumber(n2);
    HAL_Delay(2000);
    //display the second operator for 2 seconds
    if(o2 == '+') {
        operation = 0b0100001;
    } 
    else if(o2 == '-') {
    ...
    Display7Segment(operation);
    HAL_Delay(2000);
    //display the third number for 2 seconds
    Display7SegmentNumber(n3);
    HAL_Delay(2000);
}


//lights up the red LED
void lightRed(bool state){
    if (state){
...
int main(void)
{
    ...
    //declare variables
    int answer = 0, num_one, num_two, num_three, num_equation;
    bool correct = false;

    //initalizing the random variables with appropriate random values
    num_equation = (HAL_GetTick()%5)+1;
    num_one = (rand()%10);
    num_two = (rand()%10);
    num_three = (HAL_GetTick()%10);

    //switch case for which equation to use (display/compute)
    switch(num_equation){
        case 1:
            display(num_one, num_two, num_three, '+', '+');
            answer = num_one + num_two + num_three;
            //if answer is negative, get its absolute value
            if (answer < 0){
                answer *= -1;
            }
            break;
    ...
    //run the while loop as long as user isn't getting the correct answer
    while (correct == false){

    char *keypad_symbols = "123A456B789C*0#D";

    //set a boolean to true, valid will be used to check is user input is correct
    bool valid = true;
    ...
    if (answer > 100){
        ...
        while (count < 3){
            ...
            input[count] = keypad_symbols[ReadKeypad()];
    ...
    //taking input when answer is greater than 10
    ...
    //taking input when answer is less than 10
    else{
        int count = 0;
        char ans[2] = {' ', '\0'};
        sprintf(ans, "%d", answer);
        ...
        //if input is correct, light up green and end game. If it isn't, light up red and get input again
        if (valid){
            correct = true;//ends while loop
            lightGreen(true);//light up the green LED
            HAL_Delay(10000);//keep it on for 10 seconds
            lightGreen(false);//turn off the green LED
        }
        else{
            lightRed(true);//light up the red LED
            HAL_Delay(5000);//keep it on for 5 seconds
            lightRed(false);//turn off the red LED
        }
    }
    ...
    // use b or c to accou t for multiplication or such 
    ...
    //do not connect the decimal point as we will only be using integers and no division will be required
    
// void delay (unsigned int dly) ;
// void main (void) /* 0 1 6 7 8 9*/ {
//     unsigned int nums [10] ={0X3F, 0x06, 0X5B, 0X4F, 0XA6,0X6D, 0X7D, 0X07,0X7F, 0X6F};
//     unsigned int loop-0;
...
//  for (delay gen =0; delay_gen < 1000; delay_gen++); /* delay */


    //     for (int i = 0; i < 10; ++i)
    //     {
    //         Display7Segment(i);
    //         HAL_Delay(900);  // 1000 milliseconds == 1 second
    //     }
#endif
    ...
    //char *keypad_symbols = "123A456B789C*0#D";
    // note that they're numbered from left to right and top to bottom, like reading words on a page
    // figure out how to do seperate segements
